...
- Provisions public and private subnets with separate route tables
- Configures Internet Gateway, NAT Gateway, and Service Gateway for network connectivity
- Supports optional Network Security Group (NSG) for OKE control plane with customizable API endpoint access
- Implements security rules for Kubernetes API endpoint access control
- Provides comprehensive outputs for VCN, subnets, gateways, and route tables

## Basic Usage

```hcl
module "vcn" {
  source = "git::https://github.com/nullplatform/tofu-modules.git//infrastructure/oci/vcn?ref=v1.35.0"
}
```

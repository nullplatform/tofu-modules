...
Deploys and configures cert-manager with cloud provider-specific DNS01 challenge solvers for automated TLS certificate management across GCP, Azure, AWS, Cloudflare, and OCI

## Features

- Deploys cert-manager using official Jetstack Helm chart with CRDs enabled
- Configures cloud provider-specific service account annotations for workload identity integration
- Supports DNS01 ACME challenge validation across multiple cloud providers (GCP, Azure, AWS, Cloudflare, OCI)
- Creates cert-manager configuration using nullplatform Helm chart with provider-specific templates
- Deploys OCI webhook extension for Oracle Cloud Infrastructure DNS integration
- Configures recursive DNS nameservers (8.8.8.8, 1.1.1.1) for DNS01 challenge resolution
- Manages certificate issuance for both public and private domain names

## Basic Usage

```hcl
module "cert_manager" {
  source = "git::https://github.com/nullplatform/tofu-modules.git//infrastructure/commons/cert_manager?ref=v1.36.0"

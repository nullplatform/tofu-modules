resource "terraform_data" "provider_validation" {
  lifecycle {
    ...
    precondition {
      condition     = var.dns_provider_name != "azure" || !var.azure_workload_identity_enabled || length(var.azure_client_id) > 0
      error_message = "azure_client_id is required when dns_provider_name is 'azure' and azure_workload_identity_enabled is true."
    }
    precondition {

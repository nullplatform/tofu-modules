...
Deploys and configures ExternalDNS in Kubernetes with support for multiple DNS providers (Cloudflare, AWS Route53, and OCI DNS)
...
- Configures provider-specific authentication including API tokens, IAM roles, and workload identity
- Manages DNS record policies with options for create-only, sync, and upsert-only strategies
- Creates and manages Kubernetes secrets for provider credentials and configuration
- Supports both public and private DNS zone management
- Configures RBAC permissions and service accounts for provider-specific requirements

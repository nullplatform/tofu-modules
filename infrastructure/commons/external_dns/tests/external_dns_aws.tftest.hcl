...
# Validates AWS config plans successfully
run "aws_full_config" {
  command = plan
...
# Validates AWS IRSA annotation is set
...
# Validates AWS extra args include zone filtering
...
# Validates no Cloudflare secret is created for AWS
...
# Validates AWS fails without iam_role_arn
...
# Validates AWS fails without zone_id_filter
...
# Validates AWS fails with invalid zone_type

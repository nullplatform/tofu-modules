...
Creates GCP firewall rules for Istio gateways (public and private) with restricted health check port access and configurable HTTPS traffic
...
- Automatically derives network and CIDR information from GKE cluster
- Allows manual override of network name and CIDR block
- Implements deny rules to block health check port from internet
- Configures separate firewall rules for HTTPS (443) and health checks (15021)

## Basic Usage

```hcl
module "security" {
  source = "git::https://github.com/nullplatform/tofu-modules.git//infrastructure/gcp/security?ref=v1.36.0"

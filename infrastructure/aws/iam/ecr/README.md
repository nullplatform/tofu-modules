...
Provisions IAM resources for ECR image management and optional cross-account ECR pull access within a named cluster namespace. The build workflow identity (user, access key, group) lives in the `build-user` module; this module only grants ECR permissions to that group.
...
- Attaches the ECR manager policy to the shared build-workflow group (created by the build-user module) for group-based permission management
...
> created by this module. They now live in `build-user`. See that module's README for the
...
| <a name="input_build_workflow_group_name"></a> [build\_workflow\_group\_name](#input\_build\_workflow\_group\_name) | Name of the IAM group (from the build-user module) to which the ECR manager policy is attached. The build workflow user is a member of this group. | `string` | n/a | yes |
...
{
  "name": "ecr",
  "description": "Provisions IAM resources for ECR image management and optional cross-account ECR pull access within a named cluster namespace. The build workflow identity (user, access key, group) lives in the build-user module; this module only grants ECR permissions to that group.",

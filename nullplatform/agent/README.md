...
Deploys the nullplatform agent to a Kubernetes cluster via a Helm chart, configuring it for a specific cloud provider with environment-driven worker orchestration for scope deployments

## Architecture

A single helm_release resource deploys the nullplatform-agent chart from the official Helm repository, with all chart values assembled by templatefile() into a YAML document via locals. Two terraform_data resources handle an API key replacement trigger and cross-variable precondition validation (cloud-provider-specific required inputs like aws_iam_role_arn and azure_* credentials). The locals layer merges provider-specific config maps, worker k8s env patches, and ingress template paths into a unified values structure before it reaches the helm_release. Outputs surface the release name, namespace, and chart version for downstream consumption.
...
```hcl
module "agent" {
  source = "git::https://github.com/nullplatform/tofu-modules.git//nullplatform/agent?ref=v7.11.0"
...
module "agent" {
  ...
  cloud_provider                  = "aws"
  cluster_name                    = "your-cluster-name"  # Required when cloud_provider = "aws"
  image_tag                       = "your-image-tag"
  nullplatform_agent_helm_version = "your-nullplatform-agent-helm-version"
  tags_selectors                  = "your-tags-selectors"
}
...
|------|-------------|------|---------|:--------:|
| <a name="input_agent_deploy_env"></a> [agent\_deploy\_env](#input\_agent\_deploy\_env) | Publish the deploy/DNS variables (DOMAIN, DNS\_TYPE, NAMESPACE, the ingress paths, the per-cloud values including AZURE\_CLIENT\_SECRET) to the agent container.<br/><br/>On by default, because a scope running inside the agent reads them there. Turn it off once every scope runs in its own worker: workers get their own copy through worker.patches, so the agent keeps only what it needs for itself, and no cloud secret sits in its Deployment. extra\_envs and the agent's own identity (AWS\_IAM\_ROLE\_ARN) are unaffected. | `bool` | `true` | no |
...
| <a name="input_worker_orchestrator"></a> [worker\_orchestrator](#input\_worker\_orchestrator) | Configure worker orchestration: emit the chart's top-level "worker" block (backend, allowedRegistries, idleTTL and the per-package patches built from worker\_orchestrated\_packages and worker\_k8s\_packages) plus anything set in var.worker.<br/><br/>Off by default: the module writes no "worker" key and builds no patches, so the chart's own defaults apply and scopes keep running inside the agent container (the legacy exec flow, see var.agent\_repo). Turn it on to run packages in their own worker pods; var.worker, the package lists and worker\_memory\_limit are ignored while it is off.<br/><br/>The agent's own env is the same either way: DOMAIN, DNS\_TYPE, NAMESPACE and the rest are always published to the agent container. | `bool` | `false` | no |
...
{
  "name": "agent",
  "description": "Deploys the nullplatform agent to a Kubernetes cluster via a Helm chart, configuring it for a specific cloud provider with environment-driven worker orchestration for scope deployments",
  ...
  "inputs": [
    ...
    {
      "name": "worker_k8s_packages",
      "description": "Package slugs whose worker runs the k8s scope and must receive its env vars (DNS_TYPE, K8S_NAMESPACE, template paths, CLUSTER_NAME, extra_envs). Add every package that runs the k8s scope code, e.g. [\\",
      "required": false
    },
    {
      "name": "cluster_name",
  ...
  "outputs": [],
  "hash": "77df5f1e2c1bfd8b11d476abc2661975"
}
END_AI_METADATA -->

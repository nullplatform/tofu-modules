...
# Package (optional)
#
# Registers this scope definition as a versioned package: one revision whose
# bill of materials pins the service specification, every action specification,
# and the caller's artifacts. Mirrors exactly what `np package publish` pins,
# so Terraform-defined scopes and CLI-published packages are interchangeable.
#
...
resource "nullplatform_package" "this" {
  ...
  nrn        = var.nrn
  slug       = coalesce(var.package.slug, local.service_slug)
  name       = coalesce(var.package.name, var.service_spec_name)
  version    = var.package.version
  default    = var.package.default
  tags       = var.package.tags
  visible_to = local.package_visible_to

  # The service specification — the CLI publishes this component as "scope".
  components {
    name                 = "scope"
